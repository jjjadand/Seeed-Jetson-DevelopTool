...
> Adapted from the OpenClaw format for Claude Code (claude.ai/code).
...
A skill is a directory installed at `~/.claude/skills/<skill-name>/` containing:
- **`CLAUDE.md`** — the agent's instruction file (loaded when the skill is invoked)
- **`scripts/`** — executable helper scripts (bash, python)
- **`references/`** — static data files (JSON, markdown) the scripts read

Claude Code discovers skills from this path and surfaces them when the user
asks about a relevant topic.
...
Minimal skill: just `CLAUDE.md`. Add `scripts/` and `references/` only when needed.
...
The body of `CLAUDE.md` is the agent's operating manual. Structure it as:
...
The agent instruction in `CLAUDE.md` should say:
...
Every `CLAUDE.md` should include a table mapping `[STOP]` outputs to agent actions:
...
## 8. CLAUDE.md Agent Instructions Style
...
~/Dayu_ws/jetson-develop/skills/claude/my_skill/

# Installed location (Claude Code reads from here):
...
- [ ] `CLAUDE.md` has valid YAML frontmatter (`name`, `description`)
...
- [ ] `CLAUDE.md` has a failure decision table covering all `[STOP]` cases
