...
> Adapted from the OpenClaw format for the open AGENTS.md standard (agents.md).
...
A skill is a directory installed at `~/.codex/skills/<skill-name>/` containing:
- **`AGENTS.md`** — the agent's instruction file (loaded when the skill is invoked)
- **`scripts/`** — executable helper scripts (bash, python)
- **`references/`** — static data files (JSON, markdown) the scripts read

Any AGENTS.md-compatible coding agent discovers skills from this path and
surfaces them when the user asks about a relevant topic.
...
Minimal skill: just `AGENTS.md`. Add `scripts/` and `references/` only when needed.
...
description: One-sentence description shown when browsing skills.
---
```
...
The body of `AGENTS.md` is the agent's operating manual. Structure it as:
...
The agent instruction in `AGENTS.md` should say:
...
Every `AGENTS.md` should include a table mapping `[STOP]` outputs to agent actions:
...
## 8. AGENTS.md Agent Instructions Style
...
~/Dayu_ws/jetson-develop/skills/codex/my_skill/

# Installed location (agent reads from here):
...
- [ ] `AGENTS.md` has valid YAML frontmatter (`name`, `description`)
...
- [ ] `AGENTS.md` has a failure decision table covering all `[STOP]` cases
