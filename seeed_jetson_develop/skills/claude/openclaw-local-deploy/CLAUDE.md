...
description: Deploy Claude Code (Clawdbot) locally on reComputer Jetson with Ollama for a self-contained AI control hub. Covers Ollama installation, model pulling, Claude Code setup, and local model configuration. Requires Jetson with ≥16GB RAM and JetPack 6.2.
---
...
| Config file syntax error | Validate JSON: `python3 -m json.tool ~/.claude/openclaw.json`. Fix trailing commas. |
| WebUI not loading | Check gateway is running: `openclaw gateway status`. Verify port 18789 is not blocked. |
| "unable to access" error on WebUI | Token authentication is enabled. Find token in `~/.claude/openclaw.json` under `gateway.auth.token` and append to URL. |
